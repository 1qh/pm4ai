...
15. A CLAUDE.md sync is ZERO-COMPUTE, both on push and locally. On push: the generated `ci.yml`/`release.yml` carry `paths-ignore: [‘CLAUDE.md’, ‘**/CLAUDE.md’, ‘AGENTS.md’, ‘**/AGENTS.md’]`, so GitHub dispatches no runner when a push changes only those generated docs (and still runs CI when code changes alongside); when a sync commit ALSO touches a workflow yet needs no test (e.g. installing the `paths-ignore` itself), put `[skip ci]` in its message. Never `[skip ci]` a commit that changes code. Locally: `fix` SKIPS `up.sh` when the sync changed only CLAUDE.md/AGENTS.md/workflows — those cannot affect build/lint/test, so the gate would only re-verify unchanged code — which turns a docs-only fleet sync from minutes-per-repo (a fresh install + build + lint each) into seconds. Any source, package.json, tsconfig, or vendored-ui change in the same sync still runs the full gate.

...
15. A CLAUDE.md sync is ZERO-COMPUTE on push by design — the generated `ci.yml` and `release.yml` carry `paths-ignore: ['CLAUDE.md', '**/CLAUDE.md', 'AGENTS.md', '**/AGENTS.md']`, so GitHub dispatches no runner when a push changes only those generated docs (and still runs CI when code changes alongside). So a rule-source edit that regenerates every consumer’s CLAUDE.md propagates fleet-wide without firing N CI runs on one shared runner. When a sync commit ALSO touches a CI-relevant file (e.g. the workflow edit that first installs the `paths-ignore`) yet needs no test, put `[skip ci]` in that commit’s message to skip it too. Never `[skip ci]` a commit that changes code.

...
const makeTmp = async () => mkdtemp(join(tmpdir(), 'pm4ai-audit-'))
describe('audit integration', () => {
  ...
  test('detects missing root devDeps', async () => {
    const tmp = await makeTmp()
    await writeFile(join(tmp, 'package.json'), JSON.stringify({ devDependencies: {}, private: true, workspaces: [] }))
    const issues = await audit(tmp)
  ...
  test('detects non-private app packages', async () => {
    ...
    await writeFile(join(tmp, 'package.json'), JSON.stringify({ private: true, workspaces: ['apps/*'] }))
    await writeFile(join(tmp, 'apps', 'web', 'package.json'), JSON.stringify({ name: '@a/web' }))
    const issues = await audit(tmp)
  ...
  test('detects duplicate deps across workspace packages', async () => {
    ...
    await writeFile(join(tmp, 'package.json'), JSON.stringify({ private: true, workspaces: ['packages/*'] }))
    await writeFile(

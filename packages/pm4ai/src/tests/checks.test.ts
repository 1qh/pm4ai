...
describe('checkConvexSelfHosted', () => {
  ...
  test('flags process.env.NODE_ENV (dot) inside convex/', async () => {
    const tmp = makeTmp()
    mkdirSync(join(tmp, 'convex', '_generated'), { recursive: true })
    writeFileSync(join(tmp, 'convex', '_generated', 'api.d.ts'), '')
  ...
  test("flags process.env['NODE_ENV'] (single-quote bracket) inside convex/", async () => {
    const tmp = makeTmp()
    mkdirSync(join(tmp, 'convex', '_generated'), { recursive: true })
    writeFileSync(join(tmp, 'convex', '_generated', 'api.d.ts'), '')
    writeFileSync(join(tmp, 'convex', 'bar.ts'), "export const x = process.env['NODE_ENV']\n")
    writeFileSync(join(tmp, '.env'), 'CONVEX_SELF_HOSTED_URL=https://x\nSITE_URL=https://y\n')
    const issues = await checkConvexSelfHosted(tmp)
    expect(issues.some(i => i.detail.includes('NODE_ENV'))).toBe(true)
    rmSync(tmp, { recursive: true })
  })
  test('flags process.env["NODE_ENV"] (double-quote bracket) inside convex/', async () => {
    const tmp = makeTmp()
    mkdirSync(join(tmp, 'convex', '_generated'), { recursive: true })
    writeFileSync(join(tmp, 'convex', '_generated', 'api.d.ts'), '')
    writeFileSync(join(tmp, 'convex', 'baz.ts'), 'export const x = process.env["NODE_ENV"]\n')
    writeFileSync(join(tmp, '.env'), 'CONVEX_SELF_HOSTED_URL=https://x\nSITE_URL=https://y\n')
    const issues = await checkConvexSelfHosted(tmp)
    expect(issues.some(i => i.detail.includes('NODE_ENV'))).toBe(true)
    rmSync(tmp, { recursive: true })

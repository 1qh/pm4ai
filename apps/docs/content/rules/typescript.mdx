...
- ANY file containing JSX must use `.tsx`, never `.ts` — even a context provider with `<Ctx value={...}>`. biome parses `.ts` as non-JSX, misreads `<Foo>` as comparison/regex, and the fixer infinite-loops (90-min `bun run fix` hang)

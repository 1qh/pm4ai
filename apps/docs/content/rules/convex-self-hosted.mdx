...
- Never write a `convex env set` literal outside `**/sync.ts`. Cost: drift; `sync` silently overwrites it.
- Never branch on `process.env.NODE_ENV` inside `**/convex/**/*.ts`. Cost: always `'production'`, branch is dead.

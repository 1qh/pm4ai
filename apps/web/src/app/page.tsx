...
'use client'
/* oxlint-disable no-empty-function, promise/prefer-await-to-then, max-depth, no-await-in-loop, no-unmodified-loop-condition, complexity, no-use-before-define, react-perf/jsx-no-new-object-as-prop */
...
const Dashboard = () => {
  ...
  const fixMutation = useMutation({
    mutationFn: async () => client.fixAll({ all: true }),
    onError: () => toast.error('Fix failed'),
    onSuccess: () => toast.success('Fix started')
  })
  const statusMutation = useMutation({
    mutationFn: async () => client.refreshStatus({ all: true }),
    onError: () => toast.error('Status check failed'),
    onSuccess: () => toast.success('Status check started')
  })
  const stats = useMemo(
    () =>
      deriveStats({
    ...
    <div className='max-w-6xl mx-auto p-6'>
      <header className='flex items-center justify-between mb-6'>
        <div className='flex items-center gap-4'>
          ...
          {state.history.length > 1 ? (
            <span className='text-sm text-muted-foreground/60 font-mono'>{sparkline(state.history)}</span>
          ) : null}
        </div>
        <div className='flex gap-3'>
          <button
            className='px-4 py-2 bg-secondary hover:bg-accent rounded-md text-sm disabled:opacity-50 transition-colors'
      ...
      <section>
        ...
        <div className='max-h-96 overflow-y-auto space-y-1 font-mono text-xs'>
          {eventLog.length === 0 ? <div className='text-muted-foreground/40'>Waiting for events...</div> : null}
          {eventLog.map((e, i) => (
